...
# I reference this solution for help: https://stackoverflow.com/questions/65190609/cumulative-stacked-area-plot-for-counts-in-ggplot-with-r
#=== === === === === === === ===

# load relevant packages
library(tidyverse) #for data manipulation and plotting
library(viridis) # for color pallet
...
#make a date category for the visit date
raw_2 <- raw |> mutate(vis_date = as.Date(raw$Visit.Date, format = '%m/%d/%Y')) 

# drop recent visits from 2024 and get just unique sample ID, agency, and dates 
...
# I need to make a data frame with organization, year, count of events:
long_df <-df_sa |> 
  group_by(Agency, year) |> 
  summarise(count = n()) |> 
  ungroup() #ungroup at the end so that it is a tibble
...
final_dat <- long_df %>%
  ...
  mutate(aggcount = cumsum(count)) %>% #create a new cumulative count column
  ungroup()
